...
const mapYouTubeiVideoToVideo = (item: any): Video | null => {
    // CompactVideo, GridVideo, Videoなどの型に対応
    if (!item?.id || !['Video', 'CompactVideo', 'GridVideo'].includes(item?.type)) return null;
    return {
        id: item.id,
...
export async function getVideoDetails(videoId: string): Promise<VideoDetails> {
    ...
    // --- ここから修正箇所 ---
    // watch_next_feedには動画の配列や単体の動画が混在するため、flatMapで安全に展開する
    const relatedVideos: Video[] = data.watch_next_feed.flatMap((item: any) => {
        // item自体が動画オブジェクトの場合
        if (item.type === 'CompactVideo') {
            return mapYouTubeiVideoToVideo(item);
        }
        // itemが動画のリスト（シェルフなど）を持っている場合
        const innerVideos = item.videos || item.contents;
        if (Array.isArray(innerVideos)) {
            return innerVideos.map((videoItem: any) => mapYouTubeiVideoToVideo(videoItem));
        }
        return []; // 関係ない要素は空配列を返して無視
    }).filter((v): v is Video => v !== null);
    // --- ここまで修正箇所 ---
